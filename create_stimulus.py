...
class StimulusWriter:
    ...
    def get_pwls(self):
        ...
        for signal in self.input_ports:
            ...
            for bit in range(bitwidth):
                ...
                spice = f"V{signal}.{bit} {signal}.{bit} 0 PWL FILE \"{os.path.realpath(pwl_file)}\"\n"
    ...
    def make_outputs(self, lib_dir='.', output_dir='./spice', stimulus_file_name='top.sp'):
        ...
        for file in pm_files:
            shutil.copy(os.path.join(lib_dir, file), output_dir)
        ...
        with open(spice_file, 'r') as f:
            content = f.read()
        with open(os.path.join(output_dir, spice_file.split('/')[-1]), 'w') as f:
            content = '\n'.join([f'.include {f}' for f in spice_lib_files+pm_files]) + '\n' + content
            f.write(content)
        with open(os.path.join(output_dir, stimulus_file_name), 'w') as f:
